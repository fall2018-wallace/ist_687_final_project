...
plot2<-ggplot(CompOverallSat, aes(x=Airline, y=AverageCustRating,fill=factor(ifelse(area=="Southeast Airlines Co.","Highlighted","Normal")))) + geom_bar(stat="identity") + theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) +scale_fill_manual(name = "CompOverallSat", values=c("red","grey50")) 
